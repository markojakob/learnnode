...
<template>
    <nav class="pagination is-centered" role="navigation" aria-label="pagination">
        <button  @click="$emit('prev')" :disabled="!info.prev" class="pagination-previous">Prev</button>
        <button  @click="$emit('next')" :disabled="!info.next" class="pagination-next">Next</button>
        <ul class="pagination-list">
            <li v-for="page in pageNumbers">
                <span class="pagination-ellipsis" v-if="page === '...'">&hellip;</span>
                <a class="pagination-link is-current" :aria-label="`Page ${page}`" aria-current="page">46</a>
